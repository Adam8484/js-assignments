...
function roundToPowerOfTen(num, pow) {
  const tenPower = Math.pow(10, pow);
  return tenPower * Math.round(num / tenPower);
}
...
function toNumber(value, def) {
  const str = `${value}`;
  const myexp = /^\d+$/;
  if (typeof value === "object" && value instanceof Number) return value;
  if (myexp.test(str)) return parseInt(str, 10);
  return def;
}
