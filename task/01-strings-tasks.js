...
function getRectangleString(width, height) {
  const firstLine = "┌".concat("─".repeat(width - 2)).concat("┐\n");
  const lastLine = "└".concat("─".repeat(width - 2)).concat("┘\n");
  const centerLine = "│".concat(" ".repeat(width - 2)).concat("│\n");
  const middleLines = centerLine.repeat(height - 2);
...
function getCardId(value) {
  const cardValue = "A234567891JQK";
  const colors = "♣♦♥♠";
  const indexCard = cardValue.indexOf(value.charAt(0));
  const indexColor = colors.indexOf(value.charAt(value.length - 1));
  return 13 * indexColor + indexCard;
}
